...
class WP110ChartsController extends BaseController<String> {
	...
	override protected void initDaten(int stufe) {
		...
		if (stufe <= 1) {
			...
			var liste = get(
				FactoryService::wertpapierService.holeKurse(serviceDaten, wp1.uid, von, bis, wp1.datenquelle,
					wp1.kuerzel, wp1.typ, wp1.waehrung, if(k1.relativ) wp1.relationUid else null))
			...
			if (k2.dauer !== k1.dauer || !wp2.kuerzel.equals(wp1.kuerzel)) {
				...
				liste = get(
					FactoryService::wertpapierService.holeKurse(serviceDaten, wp2.uid, von, bis, wp2.datenquelle, wp2.kuerzel,
						wp2.typ, wp2.waehrung, if(k2.relativ) wp2.relationUid else null))
			}
			if (liste !== null && liste.size > 0) {
				chart2 = new PnfChart
			...
			if (k3.dauer !== k2.dauer || !wp3.kuerzel.equals(wp2.kuerzel)) {
				...
				liste = get(
					FactoryService::wertpapierService.holeKurse(serviceDaten, wp3.uid, von, bis, wp3.datenquelle, wp3.kuerzel,
						wp3.typ, wp3.waehrung, if(k3.relativ) wp3.relationUid else null))
			}
			if (liste !== null && liste.size > 0) {
				chart3 = new PnfChart
